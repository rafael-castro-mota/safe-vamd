...
vamd.acoustic_field.downwind_modes = vamd.acoustic_field.downwind_modes[0:n_modes]
time1 = perf_counter()
...
titles = ['2m horizontal', '30m horizontal']
...
for path, plot_title in zip(horizontal_validation_scan_paths, titles):
    ...
    plt.plot(ranges, 20 * np.log10(abs(ps_vamd)/ps_ref), color='orange', linestyle='-.', label='safe-vamd (w/o spread)')
    plt.plot(ranges, 20 * np.log10(abs(ps_vamd_3d)/ps_ref), color='red', linestyle='--', label='safe-vamd (w/ spread)')
    plt.xlabel("Range [m]", fontsize=12)
