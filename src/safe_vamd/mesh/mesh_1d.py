...
class Mesh1D:
    """
    A one-dimensional finite-element mesh, comprised of elements and nodes.
    ...
    **Class attributes:**
    ...
    nodes : Sequence[Node]
        A list of nodes associated with the mesh.
    elements : Sequence[Element1DArbitraryOrderLagrange]
        A list of elements associated with the mesh.
    ...
    def load_from_element_bounds(self, element_bounds: Sequence[float], degree: int) -> None:
        """
        Takes a list of heights for the elements' interfaces,
        places the appropriate interior and interface nodes (instances of *Node*) and
        creates the mesh elements (instances of *Element1DArbitraryOrderLagrange*).

        Parameters
        ----------
        element_bounds: Sequence[float]
            An ordered (ascending) list of heights (in :math:`m`) representing the bounds of the elements.
        degree: int
            The degree of the elements' piece-wise Lagrange polynomials.
    ...
    def add_top_pml(self, thick: float, n_elements: int, degree: int, tau1: float = 4, tau2: float = 4) -> None:
        ...
        elements to the top of the computational domain.

        Parameters
        ----------
        thick: float
            The thickness (in :math:`m`) of the PML.
        n_elements:int
            The number of elements in the PML.
        degree:int
            The degree of the PML elements' piece-wise Lagrange polynomials.
        tau1: float
            A unitless factor for the stretching function in the PML.
        tau2: float
            A unitless factor for the stretching function in the PML.
