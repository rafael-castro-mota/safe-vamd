...
def kirby_solver_lagrange(mesh: Mesh1D, background_field: BackgroundField, source: Source, alpha: MathFunc,
                          absor: bool = False, boundary_condition: GroundIngardMyersBC = None) -> AcousticField:
    r"""
    Assembles the finite-element matrices for the cubic eigenvalue problem
    (:math:`[A + B\lambda + C\lambda^2 + D\lambda^3]p = 0`), solves
    it and creates an acoustic field with the computed upwind and downwind vertical atmospheric (VA) modes.
    ...
    boundary_condition: GroundIngardMyersBC, optional
        An object that applies the IngardMyersBC boundary condition on the ground directly on the cubic
