...
class ModeFilter:
    """
    A class that condenses mode filtering operations for an *AcousticField* object.
    ...
    **Class attributes:**
    ...
    acoustic_field : AcousticField
        An *AcousticField* object whose mode basis are going to be filtered
    """
    ...
    def sort_by_imag_part(self, side: str = "downwind") -> None:
        """
        Sorts the modes from least attenuated to most attenuated. In the case of downwind VA modes, the least attenuated
        mode is the one with the lowest valued imaginary part. For the upwind modes, it is the one with the highest.

        Parameters
        ----------
        side: str, optional
          Set of modes to sort. (side='downwind' sorts the downwind modes, side='upwind'  the upwind modes)
    ...
    def filter_by_energy(self, inter, energy_concentration, side="downwind"):
        """
        Filters out the modes that have insufficient mode shape energy concentration within a desired height interval
        (see Eq. 20 in https://doi.org/10.1016/j.enganabound.2025.106308).

        Parameters
        ----------
        inter : Sequence[float]
            The height interval of interest (in :math:`m`).
        energy_concentration : float
            The desired energy concentration (between 0 and 1).
        side: str, optional
